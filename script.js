document.addEventListener('DOMContentLoaded', () => {
        ...
        fetch(filePath)
            ...
            .then(data => {
                ...
                newsItems.forEach(item => {
                    ...
                    } else if (item.includes(' - ')) { // Default separator check
                        [title, link] = item.split(' - ').map(part => part.trim());
                    } else {
                        console.warn(`Invalid format in: ${item}`);
                        return; // Skip items that don't have the expected format
                    }

                    // Ensure the link starts with 'https://'
                    if (!link.startsWith('https://')) {
                        console.warn(`Invalid URL detected: ${link}. Skipping...`);
    ...
    fetchNews('ConsiliumNews.txt', 'consilium-news-list', true); // Consilium News, with ////
